...
test('qoder plugin manifest exists and has required fields', () => {
  ...
  assert.equal(manifest.name, 'CURLYCODER');
...
test('qoder hooks config exists and registers UserPromptSubmit', () => {
  ...
  assert.ok(cmd.includes('CURLYCODER-mode-tracker.js'), 'must point at CURLYCODER-mode-tracker.js');
});

test('qoder rules file exists and is non-empty', () => {
  const rulesPath = path.join(root, '.qoder', 'rules', 'CURLYCODER.md');
  assert.ok(fs.existsSync(rulesPath), '.qoder/rules/CURLYCODER.md must exist');
  const content = fs.readFileSync(rulesPath, 'utf8').trim();
  assert.ok(content.length > 0, '.qoder/rules/CURLYCODER.md must not be empty');
  assert.ok(content.includes('lazy senior developer'), 'rules must contain the CURLYCODER identity');
});
...
test('qoder rules match AGENTS.md canonical body', () => {
  ...
  const qoderCopy = fs.readFileSync(path.join(root, '.qoder', 'rules', 'CURLYCODER.md'), 'utf8')
    .replace(/\r\n/g, '\n').trim();
  assert.equal(qoderCopy, canonical, '.qoder/rules/CURLYCODER.md drifted from AGENTS.md');
});

test('qoder runtime detects QODER_SESSION_ID and writes hookSpecificOutput JSON', () => {
  const { isQoder } = require('../hooks/CURLYCODER-runtime');
