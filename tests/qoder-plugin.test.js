...
test('qoder rules file exists and is non-empty', () => {
  const rulesPath = path.join(root, '.qoder', 'rules', 'CODELEAN.md');
  assert.ok(fs.existsSync(rulesPath), '.qoder/rules/CODELEAN.md must exist');
  const content = fs.readFileSync(rulesPath, 'utf8').trim();
  assert.ok(content.length > 0, '.qoder/rules/CODELEAN.md must not be empty');
  assert.ok(content.includes('lazy senior developer'), 'rules must contain the CODELEAN identity');
});
...
test('qoder rules match AGENTS.md canonical body', () => {
  ...
  const qoderCopy = fs.readFileSync(path.join(root, '.qoder', 'rules', 'CODELEAN.md'), 'utf8')
    .replace(/\r\n/g, '\n').trim();
  assert.equal(qoderCopy, canonical, '.qoder/rules/CODELEAN.md drifted from AGENTS.md');
});
