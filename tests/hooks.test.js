...
delete process.env.CODELEAN_SUBAGENT_MATCHER;
delete process.env.QODER_SESSION_ID;

const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'CODELEAN-hooks-'));
...
assert.equal(output.systemMessage, 'CODELEAN:ULTRA');
...
assert.match(
  ...
  /CODELEAN MODE ACTIVE — level: ultra/,
);
...
assert.equal(output.systemMessage, 'CODELEAN:LITE');
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /CODELEAN MODE ACTIVE — level: lite/,
);
...
assert.equal(
  ...
  'lite',
  'incidental "normal mode" in a request must not turn CODELEAN off',
);

const claudeEnv = {
  HOME: home,
  USERPROFILE: home,
  CODELEAN_DEFAULT_MODE: 'full',
};
...
result = run('CODELEAN-activate.js', {
  ...
  CODELEAN_DEFAULT_MODE: 'lite',
});
assert.equal(result.status, 0, result.stderr);
assert.equal(
  fs.readFileSync(path.join(customConfigDir, '.CODELEAN-active'), 'utf8'),
  'lite',
);
assert.equal(
  fs.existsSync(path.join(home2, '.claude', '.CODELEAN-active')),
  false,
...
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.readFileSync(path.join(copilotData, '.CODELEAN-active'), 'utf8'), 'full');
assert.equal(
  fs.existsSync(path.join(codexData, '.CODELEAN-active')),
  false,
  'copilot hooks must not write mode state to codex PLUGIN_DATA',
);
output = JSON.parse(result.stdout);
assert.match(output.additionalContext, /CODELEAN MODE ACTIVE — level: full/);
...
// (#528). Without a fallback, isCopilot was false, so CODELEAN assumed
...
// undefined path.
assert.equal(
  fs.readFileSync(path.join(vscodeHome, '.claude', '.CODELEAN-active'), 'utf8'),
  'full',
...
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.readFileSync(path.join(copilotData, '.CODELEAN-active'), 'utf8'), 'ultra');
assert.equal(
  fs.existsSync(path.join(codexData, '.CODELEAN-active')),
  false,
...
// SubagentStart hook: when CODELEAN mode is active it injects the ruleset into
...
const subFlag = path.join(subHome, '.claude', '.CODELEAN-active');
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /CODELEAN MODE ACTIVE — level: full/,
);
...
// SubagentStart scoping (issue #506): CODELEAN_SUBAGENT_MATCHER limits the
...
const scopeFlag = path.join(scopeHome, '.claude', '.CODELEAN-active');
...
const qoderState = path.join(qoderHome, '.qoder', '.CODELEAN-active');
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /CODELEAN MODE CHANGED — level: ultra/,
);
...
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.existsSync(qoderState), false, 'flag must be cleared after stop CODELEAN');
output = JSON.parse(result.stdout);
assert.equal(output.hookSpecificOutput.additionalContext, 'CODELEAN MODE OFF');

// Subagent injection via PreToolUse (task|Task matcher): when CODELEAN is
...
const mergeConfigDir = path.join(mergeHome, '.config', 'CODELEAN');
...
// #329: `/CODELEAN default <mode>` persists the default to config (survives
...
// CODELEAN_DEFAULT_MODE=review falls back to the built-in default.
const revHome = path.join(temp, 'review-default-home');
const revConfigDir = path.join(revHome, '.config', 'CODELEAN');
...
const prevEnvModeRev = process.env.CODELEAN_DEFAULT_MODE;
...
try {
  ...
  delete process.env.CODELEAN_DEFAULT_MODE;
  fs.rmSync(revConfigPath);
  process.env.CODELEAN_DEFAULT_MODE = 'review';
  assert.equal(getDefaultMode(), DEFAULT_MODE, 'CODELEAN_DEFAULT_MODE=review must fall back to the built-in default');
} finally {
  if (prevXdgRev === undefined) delete process.env.XDG_CONFIG_HOME; else process.env.XDG_CONFIG_HOME = prevXdgRev;
  if (prevEnvModeRev === undefined) delete process.env.CODELEAN_DEFAULT_MODE; else process.env.CODELEAN_DEFAULT_MODE = prevEnvModeRev;
}
