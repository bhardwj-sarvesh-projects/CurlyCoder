...
// always-on context, commands/*.toml for /CURLYCODER + /CURLYCODER-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring CURLYCODER.
...
const EXTENSION_NAME = 'CURLYCODER';
...
const REUSED_COMMANDS = ['commands/CURLYCODER.toml', 'commands/CURLYCODER-review.toml'];
const REUSED_SKILLS = ['skills/CURLYCODER/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. CURLYCODER's
...
test('manifest names the CURLYCODER extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the CURLYCODER rules', () => {
  const manifest = loadManifest();
