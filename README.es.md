<p align="center">
  <picture>
    ...
    <img src="assets/logo.png" width="220" alt="CODELEAN, el senior dev flojo">
  </picture>
</p>
...
<p align="center">
  <a href="https://CODELEAN.dev/soon"><img src="assets/waitlist-banner-es.png" alt="Algo nuevo está por llegar, únete a la lista" width="760"></a>
</p>
...
CODELEAN lo pone dentro de tu agente de IA.
...
```html
<!-- CODELEAN: el browser ya tiene uno -->
<input type="date">
```
...
<p align="center">
  <img src="assets/benchmark-agentic.svg" width="860" alt="Cada variante como porcentaje del baseline sin skill en LOC, tokens, costo y tiempo (Haiku 4.5). CODELEAN es el más bajo en cada métrica (LOC 46%, tokens 78%, costo 80%, tiempo 73%); caveman sube por encima del 100% en tokens, costo y tiempo; yagni-oneliner LOC 67%. Seguridad, tier adversarial aparte: baseline, caveman y CODELEAN 100%, yagni-oneliner 95%.">
</p>
...
El mayor esfuerzo que CODELEAN te va a pedir:
...
Abre `/plugins`, selecciona el marketplace de CODELEAN e instala CODELEAN. Luego abre `/hooks`, revisa y autoriza sus dos lifecycle hooks, y empieza un nuevo hilo.
...
Inyecta el ruleset en cada turno con el nivel activo; agrega los comandos `/CODELEAN` (ver [Comandos](#comandos)). OpenCode también carga automáticamente el `AGENTS.md` de este repo, así que las reglas aplican incluso sin el plugin. El plugin agrega los niveles `lite/full/ultra/off`.
...
Carga el ruleset como contexto permanente en cada sesión y registra los comandos `/CODELEAN`; los `skills/` también se incluyen, activados cuando una tarea los necesita.
...
Reutiliza el `gemini-extension.json` de este repo. Una diferencia: Antigravity convierte los comandos `/CODELEAN` en skills, así que los escribes en el chat (por ejemplo `/CODELEAN-review` como mensaje) en vez de seleccionarlos de un menú slash. Hasta que la migración se complete (alrededor del 18 de junio de 2026), `gemini extensions install` también funciona. Para usarlo como regla permanente, coloca el ruleset en `.agents/rules/`.
...
Instala CODELEAN como plugin de Devin; los skills quedan disponibles como `/CODELEAN:CODELEAN`, `/CODELEAN:CODELEAN-review`, etc.
...
Habilita el plugin (está desactivado por defecto): `/plugins` → Plugins → Space en `CODELEAN`, o en `~/.grok/config.toml`:
...
Abre una sesión nueva (o recarga los plugins). Los skills aparecen como `/CODELEAN`, `/CODELEAN-review`, `/CODELEAN-audit`, `/CODELEAN-debt`, `/CODELEAN-gain`, `/CODELEAN-help`. Verifica con `grok inspect`. Grok puede invocar CODELEAN automáticamente en tareas de código según la descripción del skill; usa `/CODELEAN` (o `/CODELEAN lite`, `/CODELEAN full`, `/CODELEAN ultra`) cuando necesites activarlo de forma explícita. No se usan hooks de ciclo de vida de Grok: la salida de `SessionStart` no puede inyectar instrucciones.

`AGENTS.md` sigue funcionando solo como instrucciones desde un checkout sin el plugin. Desinstalar: `grok plugin uninstall CODELEAN`.

Eso fue todo. Él estaría orgulloso. No lo va a decir.

Activo en cada sesión, con un puñado de comandos (ver [Comandos](#comandos)). `/CODELEAN ultra` existe para cuando el codebase te hizo algo personal. El texto de inicio y de cambio de modo muestra el nivel activo.

Configura el nivel para cada nueva sesión con la variable de entorno `CODELEAN_DEFAULT_MODE` (`lite`/`full`/`ultra`/`off`), o con un campo `defaultMode` en `~/.config/CODELEAN/config.json` (`%APPDATA%\CODELEAN\config.json` en Windows). El default es `full`.
...
Kiro: copia `.kiro/steering/CODELEAN.md` a `~/.kiro/steering/` (global) o `.kiro/steering/` en tu proyecto.

Fallback de GitHub Copilot CLI (modo solo instrucciones): lee `AGENTS.md` y `.github/copilot-instructions.md` en un proyecto, o copia las reglas a `~/.copilot/copilot-instructions.md` para ejecutar CODELEAN en todos tus proyectos. Esta vía mantiene la guía permanente, pero no agrega switches de modo ni hooks.
...
| Comando | Qué hace |
|---------|----------|
| `/CODELEAN [lite \| full \| ultra \| off]` | Cambia la intensidad, o apágalo. Sin argumento, reporta el nivel actual. |
| `/CODELEAN-review` | Revisa el diff actual en busca de sobre-ingeniería y devuelve una lista de qué eliminar. |
| `/CODELEAN-audit` | Audita el repo completo en busca de sobre-ingeniería, no solo el diff. |
| `/CODELEAN-debt` | Recolecta los atajos marcados con `CODELEAN:` que dejaste pendientes en un registro, para que "después" no se convierta en "nunca". |
| `/CODELEAN-help` | Referencia rápida de los comandos anteriores. |

Los comandos requieren un host compatible con skills (Claude Code, Codex, Devin CLI, OpenCode, Gemini, pi, Swival). En Codex son skills; se invocan con `@` (`@CODELEAN-review`). Los adaptadores de solo instrucciones (Cursor, Windsurf, Cline, Copilot, Kiro, Antigravity) cargan el ruleset permanente sin los comandos.
...
Sí, y deberías. Caveman achica lo que el agente dice; CODELEAN achica lo que construye. Mitades distintas, sin solapamiento: caveman deja el código intacto byte por byte, CODELEAN no se mete con la prosa. Charla concisa sobre código mínimo.

**¿Necesita un archivo de configuración?**
No. Un opcional `~/.config/CODELEAN/config.json` o la variable `CODELEAN_DEFAULT_MODE` pueden fijar el nivel default, pero nada es obligatorio.
