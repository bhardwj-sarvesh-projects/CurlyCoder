...
flag="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.CODELEAN-active"
[ -f "$flag" ] || exit 0
...
if [ -z "$mode" ] || [ "$mode" = "full" ]; then
    printf '\033[38;5;%sm[CODELEAN]\033[0m' "$color"
else
    printf '\033[38;5;%sm[CODELEAN:%s]\033[0m' "$color" "$(printf '%s' "$mode" | tr '[:lower:]' '[:upper:]')"
fi
