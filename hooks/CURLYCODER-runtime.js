...
const os = require('os');
const { getClaudeDir, getConfigDir } = require('./CURLYCODER-config');

const STATE_FILE = '.CURLYCODER-active';

// CURLYCODER: VS Code Copilot never sets COPILOT_PLUGIN_DATA — it only injects
// CLAUDE_PLUGIN_ROOT, pointed at an install path under .vscode/agent-plugins/
// (#528). Without this fallback isCopilot was false, so CURLYCODER assumed
...
// Live mode written by activate/mode-tracker. Absent flag = CURLYCODER off.
function readMode() {
  try {
...
function writeHookOutput(event, mode, context = '') {
  ...
  if (isCodex) {
    const output = { systemMessage: `CURLYCODER:${mode.toUpperCase()}` };
    if (context) {
