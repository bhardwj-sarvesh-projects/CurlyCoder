#!/usr/bin/env node
// CODELEAN — UserPromptSubmit hook to track which CODELEAN mode is active
// Inspects user input for /CODELEAN commands and writes mode to flag file

const { getDefaultMode, isDeactivationCommand, writeDefaultMode } = require('./CODELEAN-config');
const { clearMode, isQoder, readMode, setMode, writeHookOutput } = require('./CODELEAN-runtime');
const { getCODELEANInstructions } = require('./CODELEAN-instructions');
...
function finish() {
  ...
  try {
    ...
    // Match /CODELEAN commands
    let modeSwitched = false;
    let deactivated = false;
    if (/^[/@$]codelean/.test(prompt)) {
      ...
      if (cmd === '/codelean-review' || cmd === '/codelean:codelean-review') {
        mode = 'review';
      } else if (cmd === '/codelean' || cmd === '/codelean:codelean') {
        // `/CODELEAN default <mode>` persists the default to config (survives
        ...
        if (arg === 'default') {
          ...
          if (dmode === 'off' || dmode === 'lite' || dmode === 'full' || dmode === 'ultra') {
            ...
            writeHookOutput('UserPromptSubmit', dmode, 'CODELEAN DEFAULT SET — new sessions start in ' + dmode + '.');
          }
      ...
      if (isReportOnly) {
        writeHookOutput(
          'UserPromptSubmit',
          mode,
          'CODELEAN MODE ACTIVE — level: ' + mode,
        );
      } else if (mode && mode !== 'off') {
        setMode(mode);
        modeSwitched = true;
        // CODELEAN: Qoder needs the full ruleset every turn, so when a mode
        ...
        if (!isQoder) {
          writeHookOutput(
            'UserPromptSubmit',
            mode,
            'CODELEAN MODE CHANGED — level: ' + mode,
          );
        }
      } else if (mode === 'off') {
        clearMode();
        deactivated = true;
        writeHookOutput('UserPromptSubmit', 'off', 'CODELEAN MODE OFF');
      }
    }
    ...
    // SessionStart via CODELEAN-activate.js; Qoder can't, so we do it here.
    // Skip when deactivated — user just turned CODELEAN off.
    if (isQoder && !deactivated) {
      ...
      if (currentMode && currentMode !== 'off') {
        // CODELEAN: one JSON per invocation — mode-switch confirmation is
        // folded into the ruleset header so Qoder gets both in one write.
        const header = modeSwitched
          ? 'CODELEAN MODE CHANGED — level: ' + currentMode + '\n\n'
          : '';
        writeHookOutput('UserPromptSubmit', currentMode, header + getCODELEANInstructions(currentMode));
      }
    }
  } catch (e) {
    // Silent fail
