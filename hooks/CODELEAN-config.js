#!/usr/bin/env node
// CODELEAN — shared configuration resolver
//
// Resolution order for default mode:
//   1. CODELEAN_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/CODELEAN/config.json (any platform, if set)
//      - ~/.config/CODELEAN/config.json (macOS / Linux fallback)
//      - %APPDATA%\CODELEAN\config.json (Windows fallback)
//   3. 'full'
...
// "stop CODELEAN" / "normal mode" turn CODELEAN off, but only as a standalone
...
function isDeactivationCommand(text) {
  ...
  return t === 'stop codelean' || t === 'normal mode';
}

// CODELEAN: only embed the plugin install path in a statusline shell command when
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'CODELEAN');
  }
  ...
  return path.join(os.homedir(), '.config', 'CODELEAN');
}
...
function getClaudeDir() {
  // CODELEAN: CLAUDE_CONFIG_DIR overrides ~/.claude, matching Claude Code.
  return process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude');
}

function getDefaultMode() {
  // 1. Environment variable (highest priority)
  const envMode = process.env.CODELEAN_DEFAULT_MODE;
  // CODELEAN: a default must be a runtime level (off/lite/full/ultra); review is
...
// Silence the pi "CODELEAN loaded" startup toast while keeping CODELEAN active.
// CODELEAN_QUIET_STARTUP=1 (or any truthy value; 0/false/empty mean "show it")
// takes precedence, else config.quietStartup === true. Mirrors getHideStatus.
function getQuietStartup() {
  const env = process.env.CODELEAN_QUIET_STARTUP;
  if (env !== undefined) {
...
// Hide the status-bar indicator while keeping CODELEAN active (#324).
// CODELEAN_HIDE_STATUS=1 (or any truthy value; 0/false/empty mean "don't hide")
// takes precedence, else config.hideStatus === true.
function getHideStatus() {
  const env = process.env.CODELEAN_HIDE_STATUS;
  if (env !== undefined) {
    const v = env.trim().toLowerCase();
    return v !== '' && v !== '0' && v !== 'false' && v !== 'no';
  }
...
function writeDefaultMode(mode) {
  // CODELEAN: only a runtime level can be a default; review is session-only (#377).
