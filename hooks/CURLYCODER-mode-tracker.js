#!/usr/bin/env node
// CURLYCODER — UserPromptSubmit hook to track which CURLYCODER mode is active
// Inspects user input for /CURLYCODER commands and writes mode to flag file

const { getDefaultMode, isDeactivationCommand, writeDefaultMode } = require('./CURLYCODER-config');
const { clearMode, isQoder, readMode, setMode, writeHookOutput } = require('./CURLYCODER-runtime');
const { getCURLYCODERInstructions } = require('./CURLYCODER-instructions');
...
function finish() {
  ...
  try {
    ...
    // Match /CURLYCODER commands
    let modeSwitched = false;
    let deactivated = false;
    if (/^[/@$]curlycoder/.test(prompt)) {
      ...
      if (cmd === '/curlycoder-review' || cmd === '/curlycoder:curlycoder-review') {
        mode = 'review';
      } else if (cmd === '/curlycoder' || cmd === '/curlycoder:curlycoder') {
        // `/CURLYCODER default <mode>` persists the default to config (survives
        ...
        if (arg === 'default') {
          ...
          if (dmode === 'off' || dmode === 'lite' || dmode === 'full' || dmode === 'ultra') {
            ...
            writeHookOutput('UserPromptSubmit', dmode, 'CURLYCODER DEFAULT SET — new sessions start in ' + dmode + '.');
          }
      ...
      if (isReportOnly) {
        writeHookOutput(
          'UserPromptSubmit',
          mode,
          'CURLYCODER MODE ACTIVE — level: ' + mode,
        );
      } else if (mode && mode !== 'off') {
        setMode(mode);
        modeSwitched = true;
        // CURLYCODER: Qoder needs the full ruleset every turn, so when a mode
        ...
        if (!isQoder) {
          writeHookOutput(
            'UserPromptSubmit',
            mode,
            'CURLYCODER MODE CHANGED — level: ' + mode,
          );
        }
      } else if (mode === 'off') {
        clearMode();
        deactivated = true;
        writeHookOutput('UserPromptSubmit', 'off', 'CURLYCODER MODE OFF');
      }
    }
    ...
    // SessionStart via CURLYCODER-activate.js; Qoder can't, so we do it here.
    // Skip when deactivated — user just turned CURLYCODER off.
    if (isQoder && !deactivated) {
      ...
      if (currentMode && currentMode !== 'off') {
        // CURLYCODER: one JSON per invocation — mode-switch confirmation is
        // folded into the ruleset header so Qoder gets both in one write.
        const header = modeSwitched
          ? 'CURLYCODER MODE CHANGED — level: ' + currentMode + '\n\n'
          : '';
        writeHookOutput('UserPromptSubmit', currentMode, header + getCURLYCODERInstructions(currentMode));
      }
    }
  } catch (e) {
    // Silent fail
