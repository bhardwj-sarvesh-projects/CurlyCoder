#!/usr/bin/env node
// CODELEAN — Claude Code SessionStart activation hook
//
// Runs on every session start:
//   1. Writes flag file at $CLAUDE_CONFIG_DIR/.CODELEAN-active (defaults to ~/.claude; statusline reads this)
//   2. Emits CODELEAN ruleset as hidden SessionStart context
//   3. Detects missing statusline config and emits setup nudge

const fs = require('fs');
const path = require('path');
const { getDefaultMode, getClaudeDir, isShellSafe } = require('./CODELEAN-config');
...
// 2. Emit the CODELEAN ruleset, filtered to the active intensity level.
let output = getCODELEANInstructions(mode);
...
if (!isCodex && !isCopilot) try {
  ...
  const nudgeFlagPath = path.join(claudeDir, '.CODELEAN-statusline-nudged');
  if (!hasStatusline && !fs.existsSync(nudgeFlagPath)) {
    ...
    if (isShellSafe(scriptPath)) {
      ...
      output += "\n\n" +
        "STATUSLINE SETUP NEEDED: The CODELEAN plugin includes a statusline badge showing active mode " +
        "(e.g. [CODELEAN], [CODELEAN:ULTRA]). It is not configured yet. " +
        "To enable, add this to " + settingsPath + ": " +
        statusLineSnippet + " " +
        "Proactively offer to set this up for the user on first interaction.";
    } else {
      // CODELEAN: install path has shell metacharacters — don't embed it in a
