...
flag="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.CURLYCODER-active"
[ -f "$flag" ] || exit 0
...
if [ -z "$mode" ] || [ "$mode" = "full" ]; then
    printf '\033[38;5;%sm[CURLYCODER]\033[0m' "$color"
else
    printf '\033[38;5;%sm[CURLYCODER:%s]\033[0m' "$color" "$(printf '%s' "$mode" | tr '[:lower:]' '[:upper:]')"
fi
