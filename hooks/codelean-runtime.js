...
const os = require('os');
const { getClaudeDir, getConfigDir } = require('./CODELEAN-config');

const STATE_FILE = '.CODELEAN-active';

// CODELEAN: VS Code Copilot never sets COPILOT_PLUGIN_DATA — it only injects
// CLAUDE_PLUGIN_ROOT, pointed at an install path under .vscode/agent-plugins/
// (#528). Without this fallback isCopilot was false, so CODELEAN assumed
...
// Live mode written by activate/mode-tracker. Absent flag = CODELEAN off.
function readMode() {
  try {
...
function writeHookOutput(event, mode, context = '') {
  ...
  if (isCodex) {
    const output = { systemMessage: `CODELEAN:${mode.toUpperCase()}` };
    if (context) {
