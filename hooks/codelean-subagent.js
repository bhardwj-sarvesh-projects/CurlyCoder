#!/usr/bin/env node
// CODELEAN — Claude Code SubagentStart hook
//
// SessionStart context is parent-thread only and never reaches subagents, so
// without this every Task-spawned agent runs CODELEAN-unaware (issue #252).
// When CODELEAN mode is active, inject the same ruleset into each subagent.
//
// Scoping (opt-in, issue #506): set CODELEAN_SUBAGENT_MATCHER to a regex and
...
const { readMode, writeHookOutput } = require('./CODELEAN-runtime');

const mode = readMode();

// Absent flag or off → CODELEAN isn't active; inject nothing.
if (!mode || mode === 'off') {
  process.exit(0);
}
...
function inject() {
  try {
    writeHookOutput('SubagentStart', mode, getCODELEANInstructions(mode));
  } catch (e) {
...
let matcherRe = null;
try {
  if (process.env.CODELEAN_SUBAGENT_MATCHER) {
    matcherRe = new RegExp(process.env.CODELEAN_SUBAGENT_MATCHER, 'i');
  }
} catch (e) {
  matcherRe = null;
}
