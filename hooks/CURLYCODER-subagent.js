#!/usr/bin/env node
// CURLYCODER — Claude Code SubagentStart hook
//
// SessionStart context is parent-thread only and never reaches subagents, so
// without this every Task-spawned agent runs CURLYCODER-unaware (issue #252).
// When CURLYCODER mode is active, inject the same ruleset into each subagent.
//
// Scoping (opt-in, issue #506): set CURLYCODER_SUBAGENT_MATCHER to a regex and
...
const { readMode, writeHookOutput } = require('./CURLYCODER-runtime');

const mode = readMode();

// Absent flag or off → CURLYCODER isn't active; inject nothing.
if (!mode || mode === 'off') {
  process.exit(0);
}
...
function inject() {
  try {
    writeHookOutput('SubagentStart', mode, getCURLYCODERInstructions(mode));
  } catch (e) {
...
let matcherRe = null;
try {
  if (process.env.CURLYCODER_SUBAGENT_MATCHER) {
    matcherRe = new RegExp(process.env.CURLYCODER_SUBAGENT_MATCHER, 'i');
  }
} catch (e) {
  matcherRe = null;
}
