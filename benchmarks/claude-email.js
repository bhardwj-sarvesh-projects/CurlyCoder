// Email under CURLYCODER on Claude (CURLYCODER's primary target), baseline vs CURLYCODER.
const fs = require('fs'), path = require('path');
const { checkPy, pyBlock, TASKS } = require('./robustness-audit.js');
const skill = fs.readFileSync(path.join(__dirname, '..', 'skills', 'CURLYCODER', 'SKILL.md'), 'utf8');
...
(async () => {
  ...
  console.log('model                      baseline   CURLYCODER');
  for (const model of MODELS) {
    const rates = {};
    for (const [arm, sys] of [['baseline', null], ['CURLYCODER', skill]]) {
      let pass = 0, err = 0;
      for (let i = 0; i < N; i++) {
    ...
    console.log(`${model.padEnd(26)} ${rates.baseline.padEnd(10)} ${rates.CURLYCODER}`);
  }
})();
