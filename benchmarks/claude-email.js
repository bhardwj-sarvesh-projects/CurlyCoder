// Email under CODELEAN on Claude (CODELEAN's primary target), baseline vs CODELEAN.
const fs = require('fs'), path = require('path');
const { checkPy, pyBlock, TASKS } = require('./robustness-audit.js');
const skill = fs.readFileSync(path.join(__dirname, '..', 'skills', 'CODELEAN', 'SKILL.md'), 'utf8');
...
(async () => {
  ...
  console.log('model                      baseline   CODELEAN');
  for (const model of MODELS) {
    const rates = {};
    for (const [arm, sys] of [['baseline', null], ['CODELEAN', skill]]) {
      let pass = 0, err = 0;
      for (let i = 0; i < N; i++) {
    ...
    console.log(`${model.padEnd(26)} ${rates.baseline.padEnd(10)} ${rates.CODELEAN}`);
  }
})();
