...
// each file shows the same task answered with no skill vs with CURLYCODER, same model.
...
## Without CURLYCODER — ${bL} lines of code

${b.response.output.trim()}

## With CURLYCODER — ${pL} lines of code
...
with no skill (\`## Without CURLYCODER\`) and with CURLYCODER (\`## With CURLYCODER\`), so you can
