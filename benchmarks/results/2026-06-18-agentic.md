# Agentic benchmark: does CURLYCODER cut code without cutting safety?
...
[issue #126](https://github.com/bhardwj-sarvesh-projects/CurlyCoder/issues/126). His points were fair, so
this run is built to be able to *disprove* CURLYCODER, not just flatter it.

## The critique, restated honestly

The original CURLYCODER benchmark was single-shot: one prompt, one completion, count the lines.
...
2. **The baseline was a bare, chatty model.** It emitted prose, caveats, and multiple options, so
   ...
   skill. The 80â€“94% reductions were partly a conversational-baseline artifact.
...
|---|---|---|
| unit of work | one prompt â†’ one completion | a **real headless Claude Code session** in a temp workspace |
...
| arms | CURLYCODER vs bare model | baseline Â· CURLYCODER Â· caveman Â· **Colin's own one-liner prompt** |
...
An earlier agentic run showed a tiny ~4% gap and we nearly published it. It was wrong: CURLYCODER and
caveman are Claude Code **plugins** that fire a `SessionStart` hook, and that hook was firing on
*every* arm, including the baseline, so the baseline was secretly running CURLYCODER. Fixed by
...
- **Arms:**
  - `baseline`: no skill.
  - `CURLYCODER`: the skill, loaded as its real plugin.
  - `caveman`: a *terse-prose* skill (talks short, builds normally). A control: if CURLYCODER's
...
| task (ticket) | baseline | caveman | **CURLYCODER** | yagni-oneliner |
|---|--:|--:|--:|--:|
...
What this says, including where CURLYCODER does **not** win:

1. **Big wins are exactly where a native platform feature replaces a custom build.** Date picker
   âˆ’94%, color picker âˆ’92%, dropzone âˆ’62%. The baseline hand-builds a component; CURLYCODER reaches
...
2. **On irreducible code the arms converge.** Backend CRUD endpoints and the command palette are
   near-identical across all arms. CURLYCODER trims a little and never bloats, but it does not invent
   savings where there are none. An honest benchmark has to show this, and it does.
3. **caveman lands between baseline and CURLYCODER.** Terseness alone explains part of the gap but
...
Bonus: where CURLYCODER cuts code it is also cheaper and faster (date picker: ~$0.06 / 49s vs the
...
**Safe rate (5 security tasks Ã— 4 runs = 20 runs per arm):**

| arm | safe | LOC where it matters |
|---|--:|---|
| baseline | 100% (20/20) | - |
| caveman | 100% (20/20) | - |
| **CURLYCODER** | **100% (20/20)** | safe-path 9.5, sql-user 4.5 |
...
- **CURLYCODER** wrote ~9.5 lines and was safe **4/4**.

The ~3 lines CURLYCODER kept *were the path-traversal check*. "Write less" without judgment cuts the
guard; CURLYCODER's rule, *never simplify away input validation at trust boundaries*, keeps it. That
...
|---|--:|--:|--:|--:|
| caveman | âˆ’20% | +7% | +3% | +2% |
| **CURLYCODER** | **âˆ’54%** | **âˆ’22%** | **âˆ’20%** | **âˆ’27%** |
| yagni-oneliner | âˆ’33% | âˆ’14% | âˆ’21% | âˆ’30% |

**6 safety tasks** (baseline absolute, per task: 12 LOC, 104k tokens, $0.038, 22s):

| arm | LOC | tokens | cost | time | safe |
|---|--:|--:|--:|--:|--:|
| caveman | âˆ’4% | âˆ’8% | âˆ’4% | +12% | 100% |
| **CURLYCODER** | **âˆ’5%** | **âˆ’18%** | **âˆ’7%** | **âˆ’1%** | **100%** |
| yagni-oneliner | âˆ’18% | âˆ’4% | âˆ’8% | +3% | **95%** |

Reading it:

- **CURLYCODER is the only arm that cuts every metric** on the feature tasks, and the only large code
  cut (âˆ’54%). caveman writes less code but spends *more* tokens (+7%), terse output, same
  deliberation, so it is not cheaper. yagni-oneliner is cheap and fast but cuts less code than
  CURLYCODER and is the one arm that dropped a safety guard.
- The **âˆ’54% LOC is the across-task aggregate**; per task it runs from ~0% (irreducible backend
  CRUD) to âˆ’94% (date picker). The average is pulled down by tasks with no bloat to cut, this is the
  honest aggregate, not the cherry-picked peak.
- On the surgical safety tasks the code is tiny for everyone (10â€“12 lines), so size barely moves;
...
- **Nondeterminism.** `n=4`. Frontend LOC varies run to run (a custom build is 300â€“570 lines); the
...
- **Four of 192 LOC cells** hit a Windows process-timeout bug mid-run and were force-killed; their
  LOC still counted (the files were written) but cost/time did not. Every (task, arm) kept â‰¥2 of 4
...
- CURLYCODER **cuts 60â€“94% of the code** on features that have an over-build trap (custom component
...
The original 80â€“94% single-shot numbers were inflated by a chatty baseline, Colin was right. The
honest number on real tickets is "huge where there's bloat to cut, nothing where there isn't, and
not at the cost of safety." That is a smaller and more defensible claim, and it is the one CURLYCODER
