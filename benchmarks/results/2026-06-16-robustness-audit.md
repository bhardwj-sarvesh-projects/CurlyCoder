# Robustness audit: does CODELEAN degrade weak models? (2026-06-16)

Follow-up to [issue #65](https://github.com/bhardwj-sarvesh-projects/CODELEAN/issues/65). After fixing
the correctness-gate bugs, the open question was the real one: does CODELEAN's push toward
...
- Across **12 classic edge-case traps** (off-by-one, n=0, leap-century, subtractive Roman,
  deep nesting, …) on **two weak models** (`gpt-4.1-mini`, `gpt-5.4-mini`), CODELEAN holds
...
- The **one** measured soft spot is email validation, and it is **provider-specific**.
  ...
  CODELEAN's target platform, email is **100%** (haiku/sonnet/opus, n=40 each).
- The slip is **not fixable by skill text**: 8 distinct SKILL.md edits (including an n=100
  ...
  skill text that doesn't move the number is exactly the cargo-cult CODELEAN exists to avoid.

## Method

`baseline` (no skill) vs `CODELEAN` (full SKILL.md), single-shot, default params,
...
All 12 algorithmic tasks: `baseline 20/20 == CODELEAN 20/20` on **both** models. Examples
...
The one place CODELEAN measurably affects correctness is **email validation**, via the
...
**OpenAI (email, baseline vs CODELEAN, n=50–100):**
...
**Claude (email, baseline vs CODELEAN, n=40):**
...
narrow: CODELEAN writes the plain correct bool the task implies, while the unconstrained
model over-builds the interface and trips a naive `if validate(x)` caller. `url`,
`creditcard`, and `ipv4` hold at ~100% under CODELEAN on both providers, because their lazy
...
shipped. Adding skill text that doesn't work is the cargo-cult CODELEAN exists to prevent.

## Conclusion

"CODELEAN degrades model performance" is not supported. Across 12 edge-case traps, CODELEAN
