...
`baseline` (no skill) vs `CURLYCODER` (full SKILL.md), single-shot, default params,
...
All 12 algorithmic tasks: `baseline 20/20 == CURLYCODER 20/20` on **both** models. Examples
...
**Claude (email, baseline vs CURLYCODER, n=40):**
...
narrow: CURLYCODER writes the plain correct bool the task implies, while the unconstrained
model over-builds the interface and trips a naive `if validate(x)` caller. `url`,
`creditcard`, and `ipv4` hold at ~100% under CURLYCODER on both providers, because their lazy
...
"CURLYCODER degrades model performance" is not supported. Across 12 edge-case traps, CURLYCODER
