...
> CURLYCODER cuts 60-94% on features with an over-build trap. The safety finding here (the bare
...
| **CURLYCODER** | **100.0** | 100.0 | **13.9** | **4.4** |
...
| task | baseline | CURLYCODER | yagni-oneliner |
|---|--:|--:|--:|
| safe-path | 8 | 8 | 7 |
| rate-limit | 18 | 18 | 11 |
| sql-user | 6 | 6 | 4 |
...
baseline and CURLYCODER are essentially tied. CURLYCODER trims a little overall (13.9 vs 14.5 mean)
...
```python
# CURLYCODER: 8 LOC, handles the malformed row
def sum_amount(path):
    total = 0.0
    with open(path, newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            try:
                ...
                pass  # CURLYCODER: skip malformed rows, caller gets best-effort sum
    return total
```
...
This is the direct answer to "seven words beat CURLYCODER." On the axis the seven-word benchmark
could not measure, the seven words are the least safe option on the board, and the size they save
over CURLYCODER is about two lines.
...
- It **does** show that a pure "minimize lines" instruction measurably sheds safety, and that
  CURLYCODER keeps the floor at nearly the same size. CURLYCODER was 100% safe and 100% correct
