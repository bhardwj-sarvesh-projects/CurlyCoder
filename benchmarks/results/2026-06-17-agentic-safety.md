...
> measurement artifact: the CODELEAN plugin's `SessionStart` hook fired on *every* arm, so the
> "baseline" was secretly running CODELEAN, which collapsed the gap. With arms properly isolated
> (`--setting-sources project,local` + per-arm `--plugin-dir`) and a real-repo LOC tier added,
> CODELEAN cuts 60-94% on features with an over-build trap. The safety finding here (the bare
...
- With a **fair baseline** (the real coding agent, not a bare model dumping prose), CODELEAN's
...
- The interesting result is on the axis the old bench could not see. Two arms dropped safety:
  ...
  (94.4% safe). CODELEAN, baseline, and caveman stayed **100% safe**.
...
| task | baseline | CODELEAN | yagni-oneliner |
|---|--:|--:|--:|
| safe-path | 8 | 8 | 7 |
| rate-limit | 18 | 18 | 11 |
| sql-user | 6 | 6 | 4 |
...
baseline and CODELEAN are essentially tied. CODELEAN trims a little overall (13.9 vs 14.5 mean)
...
```python
# CODELEAN: 8 LOC, handles the malformed row
def sum_amount(path):
    total = 0.0
    with open(path, newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            try:
                ...
                pass  # CODELEAN: skip malformed rows, caller gets best-effort sum
    return total
```
...
This is the direct answer to "seven words beat CODELEAN." On the axis the seven-word benchmark
could not measure, the seven words are the least safe option on the board, and the size they save
over CODELEAN is about two lines.
...
- It **does** show that a pure "minimize lines" instruction measurably sheds safety, and that
  CODELEAN keeps the floor at nearly the same size. CODELEAN was 100% safe and 100% correct
