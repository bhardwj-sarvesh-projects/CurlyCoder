...
# Point CURLYCODER_TMPL at your local clone, or drop it at fixtures/full-stack-fastapi-template
# (run.py resolves a relative name under fixtures/). Mirrors the CURLYCODER_PLUGIN_DIR override.
_TMPL = os.environ.get("CURLYCODER_TMPL", "full-stack-fastapi-template")
...
# ======================================================================================
# 7. critic-email -- reproduces the critique's own example. The Scott Logic post ("CURLYCODER?
# YAGNI!", blog.scottlogic.com 2026-06-16, see issue #126) argued a bare "one-liner" prompt
# matches CURLYCODER because both shrink the line count. True on LOC -- and that is the point:
...
# "ok@ok.com\n<payload>" -- a real header/log-injection vector. CURLYCODER's rule "never simplify
...
# CURLYCODER: short, but full-string anchored -- \s excludes the newline so the injection is rejected.
EMAIL_GOOD = (
    "import re\n"
...
TASKS = {
    ...
    "vibe-langgraph": {"prompt": "Create a new file with an example of how to implement LangGraph.",
                       "score": score_vibe, "open": True},
    # candidate pool for the open/vibe set (screened baseline-vs-CURLYCODER, keep the clear winners)
