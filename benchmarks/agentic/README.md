...
instruction matches CURLYCODER, the benchmark should show it.
...
- **src_loc / src_files**: over-engineering proxy. **Tests are excluded** and tracked separately
  (`wrote_tests_rate`), since writing a test is the discipline CURLYCODER prescribes, not bloat.
...
clone of the template at the pinned commit (set `CURLYCODER_TMPL` to its path, or drop it at
...
python run.py --task tmpl-fe-datepicker,tmpl-fe-colorpicker,tmpl-fe-command,tmpl-fe-dropzone,tmpl-fe-wizard,tmpl-fe-rating,tmpl-be-duplicate,tmpl-be-search,tmpl-be-count,tmpl-be-archive,tmpl-be-bulkdelete,tmpl-be-csv \
  --arms baseline,caveman,CURLYCODER,yagni-oneliner --models haiku --runs 4 --workers 6
...
- **6 surgical safety tasks** (produced code executed against adversarial input): baseline,
  caveman, and CURLYCODER are **100% safe** (20/20); `yagni-oneliner` is **95%** (19/20), it dropped
...
> baseline was contaminated by the CURLYCODER plugin's `SessionStart` hook firing on every arm, so
> the baseline was secretly running CURLYCODER. Isolation is now enforced with `--setting-sources
