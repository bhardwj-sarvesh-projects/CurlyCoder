...
test("parseCURLYCODERCommand falls back to full when invoked bare and default is off", () => {
  assert.deepEqual(parseCURLYCODERCommand("", "off"), { type: "set-mode", mode: "full" });
});

test("parseCURLYCODERCommand parses modes, status, and default subcommand", () => {
  assert.deepEqual(parseCURLYCODERCommand("ultra", "full"), { type: "set-mode", mode: "ultra" });
  assert.deepEqual(parseCURLYCODERCommand("status", "full"), { type: "status" });
  assert.deepEqual(parseCURLYCODERCommand("default lite", "full"), { type: "set-default", mode: "lite" });
});

test("parseCURLYCODERCommand rejects review as a default (session-only mode, #377)", () => {
  assert.deepEqual(parseCURLYCODERCommand("default review", "full"), { type: "invalid", reason: "invalid-default-mode" });
});

test("resolveSessionMode still honors review as a session mode (not a default)", () => {
  const entries = [{ type: "custom", customType: "CURLYCODER-mode", data: { mode: "review" } }];
...
test("resolveSessionMode prefers latest persisted session mode", () => {
  const entries = [
    ...
    { type: "custom", customType: "CURLYCODER-mode", data: { mode: "ultra" } },
  ];
...
test("readDefaultMode and writeDefaultMode use XDG config path", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "CURLYCODER-config-"));
  const previousXdg = process.env.XDG_CONFIG_HOME;
  const previousDefault = process.env.CURLYCODER_DEFAULT_MODE;
  const configPath = join(tempDir, "CURLYCODER", "config.json");
  ...
  try {
    ...
    if (previousDefault === undefined) delete process.env.CURLYCODER_DEFAULT_MODE;
    else process.env.CURLYCODER_DEFAULT_MODE = previousDefault;
    rmSync(tempDir, { recursive: true, force: true });
  }
});

test("readQuietStartup resolves env var, config file, and default in that order", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "CURLYCODER-quiet-"));
  const previousXdg = process.env.XDG_CONFIG_HOME;
  const previousEnv = process.env.CURLYCODER_QUIET_STARTUP;
  const configDir = join(tempDir, "CURLYCODER");
  const configPath = join(configDir, "config.json");
  process.env.XDG_CONFIG_HOME = tempDir;
  delete process.env.CURLYCODER_QUIET_STARTUP;
  ...
  try {
    ...
    process.env.CURLYCODER_QUIET_STARTUP = "false";
    assert.equal(readQuietStartup(), false);
    process.env.CURLYCODER_QUIET_STARTUP = "1";
    assert.equal(readQuietStartup(), true);
  } finally {
    if (previousXdg === undefined) delete process.env.XDG_CONFIG_HOME;
    else process.env.XDG_CONFIG_HOME = previousXdg;
    if (previousEnv === undefined) delete process.env.CURLYCODER_QUIET_STARTUP;
    else process.env.CURLYCODER_QUIET_STARTUP = previousEnv;
    rmSync(tempDir, { recursive: true, force: true });
  }
});
...
test("filterSkillBodyForMode keeps rule bullets that contain a colon", () => {
  ...
  // "No unrequested abstractions:" rule or the `CURLYCODER:` comment convention)
  // contain a colon and must not be mistaken for mode-example lines.
  const skillPath = new URL("../../skills/CURLYCODER/SKILL.md", import.meta.url);
  ...
  assert.ok(filtered.includes("`CURLYCODER:` comment naming the ceiling and upgrade path"));
