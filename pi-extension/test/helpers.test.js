...
test("parseCODELEANCommand falls back to full when invoked bare and default is off", () => {
  assert.deepEqual(parseCODELEANCommand("", "off"), { type: "set-mode", mode: "full" });
});

test("parseCODELEANCommand parses modes, status, and default subcommand", () => {
  assert.deepEqual(parseCODELEANCommand("ultra", "full"), { type: "set-mode", mode: "ultra" });
  assert.deepEqual(parseCODELEANCommand("status", "full"), { type: "status" });
  assert.deepEqual(parseCODELEANCommand("default lite", "full"), { type: "set-default", mode: "lite" });
});

test("parseCODELEANCommand rejects review as a default (session-only mode, #377)", () => {
  assert.deepEqual(parseCODELEANCommand("default review", "full"), { type: "invalid", reason: "invalid-default-mode" });
});

test("resolveSessionMode still honors review as a session mode (not a default)", () => {
  const entries = [{ type: "custom", customType: "CODELEAN-mode", data: { mode: "review" } }];
...
test("resolveSessionMode prefers latest persisted session mode", () => {
  const entries = [
    ...
    { type: "custom", customType: "CODELEAN-mode", data: { mode: "ultra" } },
  ];
...
test("readDefaultMode and writeDefaultMode use XDG config path", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "CODELEAN-config-"));
  const previousXdg = process.env.XDG_CONFIG_HOME;
  const previousDefault = process.env.CODELEAN_DEFAULT_MODE;
  const configPath = join(tempDir, "CODELEAN", "config.json");
  ...
  try {
    ...
    if (previousDefault === undefined) delete process.env.CODELEAN_DEFAULT_MODE;
    else process.env.CODELEAN_DEFAULT_MODE = previousDefault;
    rmSync(tempDir, { recursive: true, force: true });
  }
});

test("readQuietStartup resolves env var, config file, and default in that order", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "CODELEAN-quiet-"));
  const previousXdg = process.env.XDG_CONFIG_HOME;
  const previousEnv = process.env.CODELEAN_QUIET_STARTUP;
  const configDir = join(tempDir, "CODELEAN");
  const configPath = join(configDir, "config.json");
  process.env.XDG_CONFIG_HOME = tempDir;
  delete process.env.CODELEAN_QUIET_STARTUP;
  ...
  try {
    ...
    process.env.CODELEAN_QUIET_STARTUP = "false";
    assert.equal(readQuietStartup(), false);
    process.env.CODELEAN_QUIET_STARTUP = "1";
    assert.equal(readQuietStartup(), true);
  } finally {
    if (previousXdg === undefined) delete process.env.XDG_CONFIG_HOME;
    else process.env.XDG_CONFIG_HOME = previousXdg;
    if (previousEnv === undefined) delete process.env.CODELEAN_QUIET_STARTUP;
    else process.env.CODELEAN_QUIET_STARTUP = previousEnv;
    rmSync(tempDir, { recursive: true, force: true });
  }
});
...
test("filterSkillBodyForMode keeps rule bullets that contain a colon", () => {
  ...
  assert.ok(filtered.includes("`CODELEAN:` comment naming the ceiling and upgrade path"));
